...
class KFXOutput(OutputFormatPlugin):
    ...
    options = {
        OptionRecommendation(
            ...
            "but because the book was not actually purchased from Amazon unexpected behavior may sometimes result. Setting this "
            "option disables some features, but reduces the chance of odd problems occurring. This setting also affects the "
            "display of cover thumbnails on Kindle devices. See the plugin documentation for details."),
    ...
    def convert(self, oeb_book, output, input_plugin, opts, log):
        ...
        #OEBWriter()(oeb_book, "C:\\Work\\KFX Output oeb")
        ...
        for optrec in EPUBOutput.options:
            if optrec.option.name not in ["insert_metadata"]:
    ...
    def cli_main(self, argv):
        ...
        intype = os.path.splitext(input)[1].lower()

        if not os.path.isfile(input):
            log.error("Input file does not exist: %s" % input)
            return
        ...
        if not output.lower().endswith(".kfx"):
            log.error("Output file must have .kfx extension")
            return
    ...
    def convert_using_previewer(self, log, book_name, input_filename, asin, cde_type_pdoc, approximate_pages,
        ...
        kpf_filename = FORCED_KPF_FILENAME or self.temporary_file(".kpf").name
    ...
    def convert_from_kpf_or_zip(self, log, book_name, input, asin, cde_type_pdoc, approximate_pages, from_kpf, output):
        ...
        book = YJ_Book(input, log, symbol_catalog_filename=get_symbol_catalog_filename())
